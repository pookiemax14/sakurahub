...
-- ESP Setup
local espBoxes, tracerLines, nameTexts = {}, {}, {}

local function projectToScreen(pos)
    local screenPos, onScreen = Camera:WorldToViewportPoint(pos)
    return (onScreen and screenPos.Z > 0) and Vector2.new(screenPos.X, screenPos.Y) or nil
end
...
local function calculateBox(player)
    ...
    local height = 7.3
    local width = (height / 5) * 1.5
    local top = root.Position + Vector3.new(0, height / 2, 0)
    local bottom = root.Position - Vector3.new(0, height / 2, 0)
    local topScreen, bottomScreen = projectToScreen(top), projectToScreen(bottom)
    if not topScreen or not bottomScreen then return nil end
    local boxHeight = (bottomScreen - topScreen).Magnitude
    local boxWidth = math.max(boxHeight / (height / width), 15)
    return Vector2.new(topScreen.X - boxWidth / 2, topScreen.Y), boxWidth, boxHeight
end
    ...
    if espEnabled then
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChildOfClass("Humanoid") and player.Character.Humanoid.Health > 0 then
                ...
                if topLeft then
                    if not espBoxes[player] then espBoxes[player] = Drawing.new("Square") espBoxes[player].Thickness = 2 espBoxes[player].Filled = false end
                    local box = espBoxes[player]
                    box.Position = topLeft box.Size = Vector2.new(boxWidth, boxHeight) box.Color = espColor box.Visible = true

                    if tracersEnabled then
                        local rootPosScreen = projectToScreen(player.Character.HumanoidRootPart.Position)
                        if rootPosScreen then
                            if not tracerLines[player] then tracerLines[player] = Drawing.new("Line") tracerLines[player].Thickness = 1 end
                            local line = tracerLines[player]
                            line.From = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y) line.To = rootPosScreen line.Color = espColor line.Visible = true
                        end
                    elseif tracerLines[player] then
                        tracerLines[player].Visible = false tracerLines[player] = nil
                    end
                        ...
                        if head then
                            ...
                            if headPosScreen then
                                if not nameTexts[player] then
                                    nameTexts[player] = Drawing.new("Text") nameTexts[player].Center = true nameTexts[player].Outline = true nameTexts[player].Font = 3 nameTexts[player].Size = 16
                                end
                                local text = nameTexts[player]
                                text.Text = player.Name text.Position = headPosScreen text.Color = espColor text.Visible = true
                            elseif nameTexts[player] then nameTexts[player].Visible = false nameTexts[player] = nil end
                        elseif nameTexts[player] then nameTexts[player].Visible = false nameTexts[player] = nil end
                    elseif nameTexts[player] then nameTexts[player].Visible = false nameTexts[player] = nil end
                else
        ...
        for _, text in pairs(nameTexts) do text.Visible = false end
    end
end)
...
local function detectSecrets()
    ...
    for _, obj in pairs(Workspace:GetChildren()) do if names[obj.Name] then return true end end
    return false
end

local function serverHop()
    if canHop and not detectSecrets() then
        canHop = false
        TeleportService:Teleport(PlaceId)
        task.delay(hopCooldown, function() canHop = true end)
    end
end
...
local Window = library:CreateWindow({
    ...
    Themeable = { Info = "Discord Server: VzYTJ7Y", Color = Color3.fromRGB(255,105,180), BackgroundImage = { Enabled = false } }
})
...
MovementSection:AddToggle({
    Name = "Speed Hack", Flag = "SpeedToggle",
    Callback = function(v) if v then enableSpeed() else disableSpeed() end end,
})
MovementSection:AddSlider({
    Name = "Walk Speed", Flag = "SpeedSlider", Value = walkSpeedValue, Min = 16, Max = 55, Precise = 1,
...
MovementSection:AddButton({
    ...
    Callback = function()
        ...
        if root then root.CFrame = root.CFrame + Vector3.new(0, 160, 0) end
    end
})
MovementSection:AddButton({
    Name = "Teleport Down",
